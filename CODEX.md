...
- Global lifecycle gate defaults to `suspended -> probing -> stable`; no full inventory read, push, recv apply, deep item traversal, or live-state write is allowed until the shallow probe is stable for `lifecycleStableTicksRequired` ticks.
- Joined clients are read-only/no-apply by default with `allowJoinedClientApply=false`; unknown roles are unsafe, and multiplayer apply remains disabled with `allowMultiplayerApply=false` until contribution ledger work is done.
